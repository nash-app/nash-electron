...
declare global {
  interface Window {
    ...
    electron: {
      // Service management
      checkInstalledServices: () => Promise<Array<{ name: string; added: boolean }>>;
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  checkXcodeCommandLineTools: () => ipcRenderer.invoke("check-xcode-command-line-tools"),
  checkHomebrewInstalled: () => ipcRenderer.invoke("check-homebrew-installed"),
  openTerminal: (command?: string) => ipcRenderer.invoke("open-terminal", command),
...
contextBridge.exposeInMainWorld("electron", {
  ...
  configureMcp: (serviceName: string) => ipcRenderer.invoke("configure-mcp", serviceName),
