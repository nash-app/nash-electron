...
import { readConfig, writeConfig, type ClientConfig } from "./config";
import { NASH_MCP_DIR, NASH_VERSION, TERMS_OF_SERVICE_URL, LLM_SERVER_RUN_COMMAND, NASH_LLM_SERVER_PORT } from "./constants";
...
// Track the LLM server process
let llmServerProcess: ChildProcess | null = null;
...
function checkHomebrewInstalled(): Promise<boolean> {
  return new Promise((resolve) => {
    const commonPaths = ["/opt/homebrew/bin/brew", "/usr/local/bin/brew"];

    const exists = commonPaths.some((path) => fs.existsSync(path));
    resolve(exists);
  });
}

// Function to open Terminal.app with optional command
function openTerminal(command?: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    // Escape single and double quotes for AppleScript
    const escapedCommand = command
      .replace(/\\/g, "\\\\")
      .replace(/"/g, '\\"')
      .replace(/'/g, "'\"'\"'");

    // Open Terminal and write the command without executing it
    const script = `
...
// Function to check if Xcode Command Line Tools is installed
function checkXcodeCommandLineTools(): Promise<boolean> {
  return new Promise((resolve) => {
    exec("xcode-select -p", { timeout: 5000 }, (error, stdout, stderr) => {
      if (error) {
        console.log("Xcode Command Line Tools check error:", error);
        resolve(false);
        return;
      }

      if (stderr) {
        console.log("Xcode Command Line Tools check stderr:", stderr);
        resolve(false);
        return;
      }
...
async function fetchTermsAndConditions(): Promise<string> {
  return new Promise((resolve, reject) => {
    https.get(TERMS_OF_SERVICE_URL, (res) => {
      let data = '';
...
function getProcessOnPort(port: number): Promise<string[]> {
  return new Promise((resolve) => {
    ...
    exec(`lsof -i :${port} | grep -i python | awk '{print $2}'`, (error, stdout) => {
      if (error || !stdout) {
        resolve([]);
        return;
      }
...
// Function to check if a process is a Python process
function isPythonProcess(pid: string): Promise<boolean> {
  return new Promise((resolve) => {
    exec(`ps -p ${pid} -o comm=`, (error, stdout) => {
      ...
      resolve(output.includes('python'));
    });
  });
}
...
async function ensurePortAvailable(port: number): Promise<void> {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (pids.length > 0) {
      console.log(`Found ${pids.length} Python process(es) using port ${port}: ${pids.join(', ')}`);
...
// Modified startLLMServer function
async function startLLMServer() {
  ...
  try {
    ...
    await ensurePortAvailable(NASH_LLM_SERVER_PORT);
    
    console.log("Starting LLM server...");
    
    // Resolve the actual path, replacing ~ with home directory
    const serverPath = LLM_SERVER_RUN_COMMAND.replace("~/", `${os.homedir()}/`);
    
    llmServerProcess = spawn("/bin/bash", ["-c", serverPath], {
    ...
    llmServerProcess.on("error", (error) => {
      console.error("Failed to start LLM server:", error);
      llmServerProcess = null;
    });

    llmServerProcess.on("exit", (code, signal) => {
      console.log(`LLM server process exited with code ${code} and signal ${signal}`);
      llmServerProcess = null;
      
      // Attempt to restart if the server crashes
      if (code !== 0 && signal === null) {
        console.log("Attempting to restart LLM server...");
        setTimeout(startLLMServer, 5000); // Wait 5 seconds before attempting restart
      }
    });
  } catch (error) {
    ...
    // Show error dialog for port conflicts
    if (error instanceof Error) {
      dialog.showErrorBox(
        "LLM Server Error",
        `Failed to start LLM server: ${error.message}`
      );
    }
  }
}
...
app.whenReady().then(() => {
  ...
  // Start the LLM server when the app starts
  startLLMServer();
  ...
  ipcMain.handle("check-installed-services", (event) => {
  ...
  ipcMain.handle("check-xcode-command-line-tools", async (event) => {
    try {
      const result = await checkXcodeCommandLineTools();
      return result;
    } catch (error) {
      console.error("Error checking Xcode Command Line Tools:", error);
      return false;
    }
  });

  ipcMain.handle("check-homebrew-installed", async (event) => {
    try {
      const result = await checkHomebrewInstalled();
      return result;
    } catch (error) {
      console.error("Error checking Homebrew:", error);
      return false;
    }
  });

  ipcMain.handle("open-terminal", async (event, command?: string) => {
    try {
      const result = await openTerminal(command);
      return result;
    } catch (error) {
  ...
  ipcMain.handle("get-tasks", async () => {
    console.log("get-tasks handler called");
  ...
  ipcMain.handle("delete-task", async (_, taskId: string) => {
    console.log("delete-task handler called");
