...
import { NASH_MCP_DIR, NASH_VERSION, TERMS_OF_SERVICE_URL } from "./constants";
...
// Configure autoUpdater for GitHub
autoUpdater.allowDowngrade = false;
autoUpdater.allowPrerelease = false;
autoUpdater.autoDownload = true;
autoUpdater.autoInstallOnAppQuit = true;
// This allows testing with draft releases during development 
if (process.env.USE_DRAFT_RELEASES === 'true') {
  autoUpdater.requestHeaders = { Accept: 'application/octet-stream' };
  autoUpdater.allowDowngrade = true;
}

// Function to check if GitHub token is present
export function checkGitHubToken(): boolean {
  const token = process.env.GITHUB_TOKEN;
  if (!token) {
    log.error('GITHUB_TOKEN environment variable is not set. Publishing will fail.');
    return false;
  }
  return true;
}
  ...
  autoUpdater.on('update-downloaded', (info) => {
    ...
    if (mainWindow) {
      ...
      dialog.showMessageBox({
        type: 'info',
        title: 'Update Ready',
        message: `${app.name} ${info.version} has been downloaded`,
