...
import { NASH_MCP_DIR, NASH_MCP_SERVER_VERSION, NASH_LOCAL_SERVER_VERSION, TERMS_OF_SERVICE_URL, NASH_LOCAL_SERVER_RUN_COMMAND, NASH_LOCAL_SERVER_PORT } from "./constants";
...
async function startLocalServer() {
  ...
  try {
    ...
    const serverPath = NASH_LOCAL_SERVER_RUN_COMMAND.replace("~/", `${os.homedir()}/`);
    ...
    localServerProcess.on("exit", (code, signal) => {
      console.log(`Local server process exited with code ${code} and signal ${signal}`);
      localServerProcess = null;
...
app.whenReady().then(() => {
  ...
  // Start the Local server when the app starts
  startLocalServer();
